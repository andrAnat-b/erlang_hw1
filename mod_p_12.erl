...
append_counted_duplicates([[N,X]|T], [], _) ->
  append_counted_duplicates([[N,X]|T], [X|[]], N - 1);

append_counted_duplicates([[N,X]|T], [V|Y], _) when (X /= V) ->
  append_counted_duplicates([[N,X]|T], [X|[V|Y]], N - 1);

append_counted_duplicates([[N,X]|T], [V|Y], C) when ((X == V) and (C > 0)) ->
  append_counted_duplicates([[N,X]|T], [X|[V|Y]], C - 1);

append_counted_duplicates([[_,X]|T], [V|Y], C) when ((X == V) and (C == 0)) ->
